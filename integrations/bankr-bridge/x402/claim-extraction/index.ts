// Bankr x402 Cloud handler for the "claim-extraction" service (bankr.x402.json). Plain
...
export default async function handler(req: Request) {
  ...
  if (!body?.whitepaperUrl) {
    return Response.json({ error: "whitepaperUrl is required" }, { status: 400 });
  }

  const res = await fetch(
    process.env.GREY_BRIDGE_URL ?? "https://api.whitepapergrey.com/v1/bridge/bankr/claim_extraction",
    {
      method: "POST",
      headers: {
