// Bankr x402 Cloud handler for the "quick-protocol-facts" service (bankr.x402.json). Plain
...
export default async function handler(req: Request) {
  ...
  if (!body?.projectQuery) {
    return Response.json({ error: "projectQuery is required" }, { status: 400 });
  }

  const res = await fetch(
    process.env.GREY_BRIDGE_URL ?? "https://api.whitepapergrey.com/v1/bridge/bankr/quick_protocol_facts",
    {
      method: "POST",
      headers: {
