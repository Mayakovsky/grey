...
- **E1-E — Curation pursuit — SUPERSEDED 2026-08-25 (bion/directive-147/150), real finding not the original premise.** Agentic.Market's curated tier sorts above the general index (70 curated services at launch), but real research found **there is no submission process at all** — no account, no form, no ToS acceptance; curation is fully automatic, driven by the richness of the metadata an endpoint already declares (`serviceName`/`tags`/`iconUrl` on the CDP-discoverable route). Checked directly against Coinbase's launch post, the x402 docs, and the live site — none describe a workflow to "submit" or "iterate on placement" the way this line originally assumed. Grey's metadata was already correct before this task was ever opened; the one real remaining lever (informal outreach via the x402 Slack/Coinbase dev channels) is Forces' own step, not engineering work — task closed `superseded` in the tasks table.
...
- **E3-B2 — Intelligence offering set, cache-only.** `prediction_market_research` — $0.10 canonical (§2.5), $0.065 realised on Olas at the 0.65× multiplier — plus `resolution_evidence_compiler`, both `CACHE_ONLY` (§2.3's compute-cost reasoning is chain-agnostic; the multiplier applies identically on Base). **`daily_tech_brief` dropped from this offering set — not built here, per Forces' explicit held-back ruling (directive-62): `enabled: false`, deliberately excluded from every real channel (x402, CDP/Bazaar, MCP, ACP, and — as actually built — mech too), reconfirmed unpurchasable directly against the live channels as of directive-148 (2026-08-25).** This line originally said to "expose, don't re-author" it; that instruction is stale relative to the real, current, deliberate decision and was never carried out — `GREY_MECH_REGISTERED_TOOLS`/`MECH_OFFERING_SLUGS` (as actually shipped) list only `prediction_market_research`/`resolution_evidence_compiler`. **`resolution_evidence_compiler` has no canonical price anywhere in this document** — unlike `prediction_market_research`, it isn't in the v7 carryover (§2.5) or the original offering catalog; authoring its canonical `@grey/schemas` price is part of this task, not a given. Don't inherit or guess a number from `prediction_market_research`'s — price it on its own merits, same evidence-based approach as OD-1's Skyfire repricing. Settlement is Marketplace-native tokens on Base, not x402 — cost ledger (E1-F) needs a conversion path for these proceeds even though no bridge is involved. G3 coverage: extend `CACHE_ONLY_SLUGS` (`packages/grey-core/test/cacheOrLive.test.ts`) for the two new slugs as an acceptance criterion here.
